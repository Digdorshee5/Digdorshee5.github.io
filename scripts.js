  ...
  function frame(){
    ...
    for(let i=0;i<particles.length;i++){
      for(let j=i+1;j<particles.length;j++){
        ...
        if(d2 < 12000){
          ctx.beginPath();
          ctx.strokeStyle = `rgba(34,211,238,${0.01 * (12000 - d2)})`;
          ctx.lineWidth = 0.5;
          ctx.moveTo(a.x, a.y);
          ctx.lineTo(b.x, b.y);
          ctx.stroke();
...
(() => {
  ...
  if(!el) return;
  // EDIT these phrases to match your role
  const phrases = [
  'I build scalable data pipelines.',
  'I solve complex data engineering problems.',
  'I turn raw data into reliable data products.',
  'I design production-grade data workflows.',
  'I build with Azure, Databricks, and PySpark.',
  'I am growing at the intersection of Data Engineering and GenAI.'
  ];
...
(() => {
  // CHANGE this date to when you started your professional career
  const base = new Date('2025-02-24T00:00:00+05:30');
...
(() => {
  ...
  function openCert(data){
    const fallbackLink = link.getAttribute('href') || '#';
    img.src = data.img;
    img.alt = data.title;
    title.textContent = data.title;
    desc.textContent = data.desc || '';
    link.href = data.link || fallbackLink;
...
(() => {
  window.copyContact = async function(text, button) {
    try {
      // Try modern Clipboard API first
      if (navigator.clipboard && window.isSecureContext) {
        await navigator.clipboard.writeText(text);
      } else {
        // Fallback for local files (file://) and older browsers
